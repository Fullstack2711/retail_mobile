...
import { updateSavedVisitorSurvey } from '../../services/visitorSurvey'
...
type Props = NativeStackScreenProps<SellerHistoryStackParamList, 'EditSavedSurvey'>
type UiStatus = 'bought' | 'not_bought'

function initialUiStatus(status: Props['route']['params']['status']): UiStatus {
  if (status === 'bought') return 'bought'
  return 'not_bought'
}

export default function EditSavedSurveyScreen({ navigation, route }: Props) {
  const { visitorId, dateTime, visitorImage, status: initialStatus, comment: initialComment } =
    route.params
  const { colors } = useTheme()
  const { t } = useLanguage()
  const [status, setStatus] = useState<UiStatus>(initialUiStatus(initialStatus))
  const [comment, setComment] = useState(initialComment?.trim() ?? '')
  const [saving, setSaving] = useState(false)

  const handleSave = async () => {
    setSaving(true)
    try {
      await updateSavedVisitorSurvey(visitorId, uiStatusToApi(status), comment)
      devLog('[VisitorSurvey] Yangilandi:', { visitorId, status })
      navigation.popToTop()
    } catch (e) {
      const message = e instanceof ApiError ? e.message : t.seller.saveError
      devLog('[VisitorSurvey] Yangilash xato:', e)
